-- ============================================================
-- FIX: delete_my_account() GAGAL untuk user yang punya coin_ledger.
--
-- Gejala: user dengan riwayat koin (bonus/gift/quest) menekan "Hapus Akun"
-- → error `coin_ledger is append-only` (trigger `coin_ledger_no_delete`
-- memblokir DELETE). Terdampak 177 user (semua yang punya ledger).
--
-- Akar: migrasi 20260911000000 memanggil
--   `delete from public.coin_ledger where user_id = v_uid;`
-- tanpa menonaktifkan trigger append-only. Fungsi lain yang sah menghapus
-- ledger (admin_delete_chat, hapus dummy, purge) memakai pola
-- `set local session_replication_role = 'replica'` — pola itu TERLEWAT.
--
-- Fix: bungkus blok hapus-ledger dengan matikan trigger sementara
-- (session_replication_role=replica) lalu kembalikan ke 'origin', persis
-- pola yang sudah terbukti (20260815040000_admin_delete_chat_coinledger_fix).
-- Definisi LIVE diambil apa adanya + hanya menambah blok tersebut.
--
-- Tidak menyentuh fungsi FROZEN (delete_my_account bukan anggota
-- scripts/frozen_functions.txt).
-- ============================================================
...
begin
  ...
  -- 5. Ledger koin & panggilan.
  --    PRIVASI/EKONOMI: coin_ledger terpasang trigger append-only
  --    (coin_ledger_no_delete -> coin_ledger_no_mutate) yang MENOLAK DELETE.
  --    Matikan sementara selama pembersihan ini (session_replication_role
  --    = 'replica' hanya berlaku di transaksi ini), lalu kembalikan 'origin'.
  --    Tanpa ini, user ber-koin gagal menghapus akun (bug 177 user).
