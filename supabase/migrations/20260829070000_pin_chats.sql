...
-- RPC pin/unpin (security definer, check participants)
...
begin
  ...
  if not (me = any(r.participants)) then raise exception 'Forbidden'; end if;

  if p_pin then
    update public.private_chats
    set pinned_by = array(select distinct unnest(array_append(coalesce(pinned_by,'{}'), me))),
        pinned_at = coalesce(pinned_at,'{}'::jsonb) || jsonb_build_object(me, now()::text)
    where chat_id = p_chat_id;
  else
    update public.private_chats
    set pinned_by = array_remove(coalesce(pinned_by,'{}'), me),
        pinned_at = coalesce(pinned_at,'{}'::jsonb) - me
    where chat_id = p_chat_id;
  end if;
