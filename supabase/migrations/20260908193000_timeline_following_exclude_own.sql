-- list_posts scope 'following': JANGAN sertakan post sendiri.
-- Sebelumnya branch following memuat `p.author_id = me` sehingga postingan
-- user tampil di tab Mengikuti (padahal ada tab Postinganku dan path
-- realtime client sudah mengecualikannya). Hapus kondisi itu — following
-- kini murni post dari yang di-follow.
-- (Salinan penuh dari 20260905170000, satu-satunya ubahan di branch scope.)
      ...
      and (
        ...
        or (p_scope = 'following' and
            exists (select 1 from public.follows f
              where f.followee_id = p.author_id and f.follower_id = me)
           )
      )
