-- ============================================================
-- ChatYuk Timeline — scope 'mine' (postingan sendiri)
--
-- ⚠️ CARA APPLY: sama seperti timeline_perf.sql — TIDAK lewat
--    `supabase db push` (CLI hang di Mac ini). DITERAPKAN MANUAL
--    via Supabase Dashboard → SQL Editor.
-- ============================================================

-- RPC list_posts v4 — tambah scope 'mine':
--   - 'all'       → semua post yang visible
--   - 'following' → post sendiri + post yang di-follow
--   - 'mine'      → HANYA post punya user sendiri
-- Perubahan: validasi p_scope + kondisi filter scope di WHERE.
    ...
    where
      ...
      and (
        ...
        or (p_scope = 'following' and exists (
              select 1 from public.follows f where f.followee_id = p.author_id and f.follower_id = me
            ))
      )
