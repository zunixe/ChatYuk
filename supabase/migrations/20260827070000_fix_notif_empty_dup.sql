-- ============================================================
-- Fix: notif kosong & double missed call
-- - kosong: nickname '' tidak ter-coalesce ke fallback, gunakan nullif
-- - double: cegah 2 missed_call dalam 30 detik untuk chat yang sama
-- ============================================================
...
begin
  begin
    if new.type = 'call' then
      -- dedup: jika ada missed_call untuk chat sama dalam 30 detik terakhir, skip
      if exists (
      ...
      -- nullif agar '' tidak lolos coalesce
...
-- juga perbaiki call_push agar nama kosong tidak lolos
...
-- perbaiki notify_call_ringing juga pakai nullif
...
begin
  ...
  perform public.call_push(new.callee_id, new.id, new.caller_id, coalesce(nullif(caller_name,''), 'User'), new.call_type);
  return new;
