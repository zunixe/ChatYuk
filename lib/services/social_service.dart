...
class SocialService {
  ...
  Future<Map<String, dynamic>> unfollowUser(String targetUid) async {
    final res = await _sb.rpc('unfollow_user', params: {'p_followee': targetUid});
    return _map(res);
  }

  Future<Map<String, dynamic>> sendFriendRequest(String targetUid) async {
    final res = await _sb.rpc('send_friend_request', params: {'p_to': targetUid});
    return _map(res);
  }

  Future<Map<String, dynamic>> respondFriendRequest(int requestId, bool accept) async {
    final res = await _sb.rpc('respond_friend_request',
  ...
  Future<Map<String, dynamic>> unsubscribeCreator(String creatorUid) async {
    final res = await _sb.rpc('unsubscribe_creator', params: {'p_creator': creatorUid});
    return _map(res);
  }

  Future<Map<String, dynamic>> setSubscriptionPrice(int price) async {
    final res = await _sb.rpc('set_subscription_price', params: {'p_price': price});
    return _map(res);
  }

  Future<Map<String, dynamic>> mySocialStatus(String otherUid) async {
    final res = await _sb.rpc('my_social_status', params: {'p_other': otherUid});
    return _map(res);
  }

  Future<List<Map<String, dynamic>>> socialList(String kind, String userUid,
      {int limit = 50}) async {
    try {
      final res = await _sb.rpc('social_list',
